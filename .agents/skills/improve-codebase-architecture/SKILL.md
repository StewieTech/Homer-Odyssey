...
Write a self-contained HTML file to `docs/architect/architecture-review-<timestamp>.html` so each run gets a fresh file in a durable repo location. Create `docs/architect` lazily if it does not exist yet. Open it for the user — `xdg-open <path>` on Linux, `open <path>` on macOS, `start <path>` on Windows — and tell them the absolute path.
